/*
 * banner轮播组件
 * 2015-06-26 14:00
 * params:cfg
 * */
function Banner() {
    this.cfg = {
        id: '',
        btnListId:'',
        btnListClass: 'select',
        thisShow: 1,
        time: 3000,
        callback: null,
        timer: null,
        ban: null,
        banList: null,
        btnList: null,
        nextBtn: null,
        prevBtn: null,
        isAuto: true
    };
    var $this = this;
    this.onStart = function () {
        $this.cfg.banList.stop(true, true).fadeOut(800).eq($this.cfg.thisShow).stop(true, true).fadeIn(800);
        //配置预览大图垂直水平居中
        var thisLi = $this.cfg.banList.eq($this.cfg.thisShow);
        var thisimg = thisLi.find('a.photoCon img');
        var thisimgH = thisimg.height();
        thisimg.css('marginTop', ((thisLi.height()-thisimgH)/2-10) +'px');
        if ($this.cfg.btnList) {
            $this.cfg.btnList.removeClass($this.cfg.btnListClass).eq($this.cfg.thisShow).addClass($this.cfg.btnListClass);
        }
        if($('.pageNow').size() != 0){
            $('.pageNow').html($this.cfg.thisShow+1);
        }
        if($this.cfg.isAuto){
            if ($this.cfg.thisShow >= $this.cfg.banList.length - 1) {
                $this.cfg.thisShow = 0;
            } else if ($this.cfg.thisShow < -1) {
                $this.cfg.thisShow = $this.cfg.banList.length - 1;
            } else {
                $this.cfg.thisShow = $this.cfg.thisShow + 1;
            }
        }
    },
	this.stop = function () {
	    if ($this.cfg.timer) {
	        clearInterval($this.cfg.timer);
	    }
	},
	this.start = function () {
	    $this.cfg.timer = setInterval($this.onStart, $this.cfg.time);
	},
	this.gotos = function (index) {
        $this.stop();
        $this.onStart();
        if($this.cfg.isAuto){
            $this.start();
        }
	}
}

Banner.prototype = {
    init: function (cfg) {
        var $this = this;
        $this.cfg = $.extend($this.cfg, cfg);
        $this.cfg.ban = $($this.cfg.id);
        $this.cfg.banList = $this.cfg.ban.find('li');
        $this.cfg.banList.hide();
        if($this.cfg.thisShow == 0){
            $this.gotos(1);
        }else{
            if($('.pageNow').size() != 0){
                $this.gotos($this.cfg.thisShow);
            }else{
                $this.cfg.banList.eq(0).show();
            }
        }
        $this.cfg.btnList = $($this.cfg.btnListId).find('li');
        $this.stop();
        if($this.cfg.isAuto){
            $this.start();
        }
            //一张以上生成分页按钮，只有一张时停止切换
        var bansize = $this.cfg.banList.length;
        if (bansize > 1) {
            //自动生成分页按钮
            /*
	    	for(var i = 0; i < bansize; i++){
	    	 	if(i == (bansize-1)){			
	    	 		btnList.append('<li class="nobg"><span></span></li>');
	    	 	}else{
		    		btnList.append('<li><span></span></li>');
		    	}
		    }*/
            $this.cfg.btnList.removeClass($this.cfg.btnListClass).eq(0).addClass($this.cfg.btnListClass);
            $this.cfg.btnList.each(function (index) {
                $(this).off('click').on('click', function (e) {
        			$this.cfg.thisShow = index;
        			$this.cfg.btnList.removeClass($this.cfg.btnListClass).eq(index).addClass($this.cfg.btnListClass);
                    $this.gotos($this.cfg.thisShow);
                    e.stopPropagation();
                });
            });
        } else {
            $this.stop();
        }
        
        if($this.cfg.nextBtn){
            $($this.cfg.nextBtn).off('click').on('click', function (e) {
                //新加，解决bug#1881，轮播到第二张跳转不了
                if ($this.cfg.thisShow >= $this.cfg.banList.length - 1) {
                    $this.cfg.thisShow = 0;
                } else if ($this.cfg.thisShow < -1) {
                    $this.cfg.thisShow = $this.cfg.banList.length - 1;
                } else {
                    $this.cfg.thisShow = $this.cfg.thisShow + 1;
                }
                // console.log($this.cfg.thisShow);
                // if ($this.cfg.thisShow >= $this.cfg.banList.length - 1) {
                //     $this.cfg.thisShow = 0;
                // } else{
                //     $this.cfg.thisShow = $this.cfg.thisShow + 1;
                // }
                $this.gotos($this.cfg.thisShow);
            e.stopPropagation();
        });
        }
        if($this.cfg.prevBtn){
            $($this.cfg.prevBtn).off('click').on('click', function (e) {
                $this.cfg.thisShow = $this.cfg.thisShow - 1;
                if ($this.cfg.thisShow < 0) {
                    $this.cfg.thisShow = $this.cfg.banList.length - 1;
                }
                $this.gotos($this.cfg.thisShow);
                e.stopPropagation();
            });
        }
        return this;
    }
}