...
function checkzcuname() {
    ...
    if (uname.match(reg)) {
        $.post("../usersServlet",{op:"checkname",zcname:uname},function(data){
           if(data==0){
...
function checkzcemail() {
    ...
    if (zcemail.match(reg)) {
    	$.post("../usersServlet",{op:"checkemail",email:zcemail},function(data){
            if (data==0) {
...
function checkzccode(){
	...
	$.post("../usersServlet",{op:"checkcode",code:zccode},function(data){
        if (data>0) {
            $("#zccodep").html("验证码正确...").css("color", "#0F0");
        } else {
            $("#zccodep").html("无效的验证码...").css("color", "#F00");
            $("#zccode").val("");
        }
    });
}
