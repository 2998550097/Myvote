...
insert into VoteItem (viId, voId, vsId,vuId)
values (428, 408, 101, 100001);
create sequence seq_user_id start with 100010;

select * from VOTEUSER;
select * from VOTEOPTION;
select * from VOTEOPTION;
select * from VOTEITEM;
count(distinct(vuId)) count
select count(distinct(vuid)) from voteitem where vsid=101 union
select rownum,s.vstitle,o.vooption,count(vuid) from VOTESUBJECT s,VOTEOPTION o,VOTEITEM i 
where s.vsId=o.vsId and s.vsId=i.vsId and 
o.voId=i.voId and i.vsId=101;

delete from voteuser where vuusername='tt'
alter table voteuser modify vuEmail varchar2(50) not null;

alter table voteuser add constraint uk_voteuser_vuname unique(vuUsername);
alter table voteuser add constraint uk_voteuser_vuemail unique(vuEmail);
select count(distinct(vuId)) count from VoteItem;

select o.*,(select count(distinct(vuId))  from VoteItem  where void=o.void) optionCount from voteoption o where vsId=103;

select s.*,(select count(distinct(vuId))  from VoteItem  where vsid=s.vsid) userCount,
